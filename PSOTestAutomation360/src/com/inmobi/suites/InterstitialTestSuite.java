...
import com.inmobi.psoapp360.test.InterstitialTests360;
		...
		for ( String device : devicesToExec ) {
			if ( device.equalsIgnoreCase("smartphone") ) {
					...
					for (String boltType : boltTypeListPortrait) {
						for (String slotSize : slotListPortrait) {
							for (String creativeSize : creativeListPortrait) {
								...
								suite.addTest(TestSuite.createTest(InterstitialTests360.class, testCaseName));	// adding individual tests using loop
							}
						}
					}
				} 
				...
				if ( flexiMode ) {
					...
					suite.addTest(TestSuite.createTest(InterstitialTests360.class, "testInterstitialAds_Async_Slot320x480_Creative320x480_FlexiTest"));	// adding individual tests using loop
				}
